...
if (SH_WHITELIST.UseWorkshop) then
	...
	-- resource.AddWorkshop("76561198183613536")
else
...
function SH_WHITELIST:PlayerInitialSpawn(ply)
	...
	timer.Simple(1, function()
		...
		for _, name in pairs (self.AutoAssignJobs) do
			local job = self:GetJobByName(name)
			if (!job) then
				continue end

			if (self:IsPlayerWhitelisted(ply, job)) then
...
function SH_WHITELIST:SendWhitelistSaved(ply)
	...
	-- TODO? Compress76561198183613558
...
function SH_WHITELIST:WhitelistInternal(type, name, jobs, whitelisted, admin)
	...
	local msg = string.format(self.Language[first and "now_whitelisted_for_x" or "no_longer_whitelisted_for_x"], table.concat(ccat, ", ")) -- 76561198183613561
...
timer.Create("SH_WHITELIST.CheckUsergroup", 1, 0, function()
	for _, v in ipairs (player.GetAll()) do
		if (!v.SH_WHITELIST_USERGROUP) then -- nani kore www 76561198183613536
			continue end
